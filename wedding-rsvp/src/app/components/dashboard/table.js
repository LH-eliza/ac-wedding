...
const RSVPTable = forwardRef((props, ref) => {
    ...
    if (groupName.trim()) {
      setGroupMembers((prevMembers) =>
        prevMembers.map((member) => ({
          ...
          lastName:
            member.lastName === "" ||
            prevMembers.some((m) => m.lastName === member.lastName)
              ? groupName.trim()
              : member.lastName,
        }))
      );
  ...
  const filteredGroupedInvitations = Object.entries(groupedInvitations).reduce(
  ...
  const getGroupSummary = (members) => {
    ...
    const declinedCount = members.filter(
      (m) => m.rsvpStatus === "Declined"
    ).length;
    const pendingCount = members.filter(
      (m) => m.rsvpStatus === "Pending"
    ).length;
  ...
  const saveEdit = async (individualId) => {
    try {
      ...
      if (response.ok) {
        ...
        setIndividuals(
          individuals.map((ind) =>
            ind._id === individualId
              ? { ...ind, ...editForm, updatedAt: new Date().toISOString() }
              : ind
          )
        );
  ...
  const deleteIndividual = async (individualId, individualName) => {
    if (
      !confirm(
        `Are you sure you want to delete ${individualName}? This action cannot be undone.`
      )
    ) {
      return;
    }
  ...
  const addMemberToGroup = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/rsvp/group/${selectedGroupCode}/member`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const exportToCSV = () => {
    ...
    const csvData = individuals.map((individual) => [
      ...
      individual.updatedAt
        ? new Date(individual.updatedAt).toLocaleDateString()
        : "",
    ]);
  ...
  const handleAddGroupMember = () => {
    ...
    setGroupMembers([
      ...groupMembers,
      { firstName: "", lastName: defaultLastName },
    ]);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="w-full max-w-none">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  ...
                  groups •{" "}
                  {Object.values(filteredGroupedInvitations).flat().length}{" "}
                  individuals
                  {searchTerm &&
                    ` (filtered from ${
                      Object.keys(groupedInvitations).length
                    } groups)`}
                </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {["Accepted", "Declined", "Pending"].map((status) => {
                    const count = individuals.filter(
                      (individual) => individual.rsvpStatus === status
                    ).length;
                    return (
                      <div
                        key={status}
              ...
              {(() => {
                ...
                return (
                  dietaryStats.length > 0 && (
                    <div data-testid="dietary-stats">
                      <h3 className="text-sm font-medium text-gray-700 mb-2">
                        Dietary Restrictions (Accepted Guests)
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {dietaryStats.map(([diet, count]) => (
                          <div
                            key={diet}
                            className="bg-white rounded-full shadow px-3 py-1 text-center border"
                          >
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {Object.entries(filteredGroupedInvitations).map(
                  ([invitationCode, members]) => {
                      ...
                      <React.Fragment key={invitationCode}>
                        ...
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <span
                                className="mr-2 text-lg text-gray-700 font-bold"
                          ...
                          <td className="px-6 py-4">
                            ...
                            <div className="text-xs text-gray-500">
                              {summary.totalMembers} member
                              {summary.totalMembers !== 1 ? "s" : ""}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-500">
                            {formatDate(
                              Math.min(
                                ...members.map((m) => new Date(m.createdAt))
                              )
                            )}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-500">
                        ...
                        {isExpanded &&
                          members.map((individual) => {
                            const isEditing =
                              editingIndividual === individual._id;
                              ...
                              >
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-gray-900">
                                  {isEditing ? (
                                    ...
                                    <span
                                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRsvpStatusColor(
                                        individual.rsvpStatus
                                      )}`}
                                ...
                                <td className="px-6 py-4 text-sm text-gray-900">
                                  {isEditing ? (
                                    ...
                                    individual.dietaryRestrictions.length >
                                      0 ? (
                                    <span data-testid="dietary-restrictions">
                                      {individual.dietaryRestrictions.join(
                                        ", "
                                      )}
                                    </span>
          ...
          {Object.keys(filteredGroupedInvitations).length === 0 &&
            searchTerm && (
              <div className="text-center py-12">
                <div className="text-gray-500 text-lg">
      ...
      >
        <div className="relative">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Adding to group:{" "}
            <span className="font-semibold">#{selectedGroupCode}</span>
          </p>
