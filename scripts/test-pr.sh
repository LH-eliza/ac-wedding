...
if [ ! -f "wedding-rsvp/package.json" ]; then
    print_error "wedding-rsvp/package.json not found. Please run this script from the project root."
    exit 1
fi
...
echo "- cd wedding-rsvp && npm run test:e2e:open    # Open Cypress runner"
echo "- cd wedding-rsvp && npm run test:e2e         # Run e2e tests headless"
echo "- cd wedding-rsvp && npm run lint             # Run linting"
